...
void ReadPD(){
	...
	delay(5); // 5 ms wait for things settling down
...
void Move(){
    ...
    // use limit switch to stop motor

	tempSpeed = limit_pwm(steer + speed);
	motor[port1] = tempSpeed;
	tempSpeed = limit_pwm(steer - speed);
	motor[port10] = tempSpeed;
}

// we can run this in main.c to better managa states.
